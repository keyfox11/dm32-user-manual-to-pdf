...
cached afterwards, so later builds are just the render. That is roughly a minute for two
full passes plus a verification pass.
...
yourself a readable copy of a document that is already published for free. Please don't use
it to redistribute SwissMicros' work.
...
| `--all` | — | Every chapter. This is already the default, and is accepted so that saying it explicitly is not an error. |
...
range, as in `dm32_user_manual_ch4-6.pdf`. A range matching no chapters is an error rather
than an empty PDF.
...
That avoids a parsing dependency. Much more importantly, it means the pipeline can
...
To make the measurement meaningful, the viewport is set to the print content box, which is
7.1 × 9.6 in at 96 dpi for the default margins. The browser then lays the document out at the
...
| No heading ends a page | Otherwise its content is orphaned overleaf. The site's own CSS covers h2/h3 only, and h4 appears 178× |
...
a `<ul>` breaks between its `<li>` children. That is exactly how a single bullet ends up
...
silently makes every list of three or fewer items atomic. That is harmless for a three-line
bullet list and ruinous for a three-step procedure with a screenshot in each step. One such
list, 8.4 in tall, ejected itself onto a fresh page and stranded 5.5 in behind it. So
`render.mjs` measures first and welds only when what it would drag along is under
`--weld-ratio` (25%) of page height. Table rows go through the same path.
...
emits a bookmark entry per heading, and each entry's destination names its page directly. The
outline therefore doubles as the heading-to-page map.
...
4. **Verify**: re-read the second pass and confirm every printed number is the page that
   heading actually ended up on. Reported on every run, and a mismatch fails the build.
...
npm package instead. That is the whole tree: loader, jax, extensions and web fonts. It pins the
...
miss. That is `.br`, "inline style brown", marking the stack registers and `▲`. It needs its
own slot because `▲` is labelled `.or` 16 times and `.br` 63 times. Headings, key chips,
...
Stripes against dots rather than two stripe angles. That is a categorical difference rather
than a matter of degree, so one seen alone still identifies itself.
...
marker then looked like a different symbol depending on where the line happened to break.
...
linear light, so every source pixel survives. That is verified identical to the colour
build at 1481 px max, 1241 px median. Images already greyscale are detected and skipped
...
Typography is the real lever, and 10 pt is already fairly tight for a reference manual. That is
why the default keeps the figures large.
...
- **12 cross-references do not resolve.** They are broken in the source document and dead on
  the website too. The targets include `#Loops with counters (DSE` (a malformed AsciiDoc xref)
  and `#File`, `#Settings`, `#battery`, which no element defines. The render reports the count
  every time. It should read 12 for a complete build, and any increase means something here
  dropped an anchor.
...
- Blue shift labels (`.bl`) are the one colour that departs from the source. Bold 10 pt
  `#97B6E6` is washed out on white paper, so it moves to `#4A82D6`. That keeps the same hue and
  saturation, raised to sit closer to the orange beside it. Pass `--shift-blue "#97B6E6"` for
